...
if %errorlevel% neq 0 (
    ...
    powershell -Command "Invoke-WebRequest -Uri 'https://github.com/git-for-windows/git/releases/download/v2.47.1.windows.2/Git-2.47.1.2-64-bit.exe' -OutFile '%TEMP%\git-installer.exe'" 2>nul
    if not exist "%TEMP%\git-installer.exe" (
        echo ERROR: Failed to download Git. Check your internet connection.
        echo You can install it manually from https://git-scm.com
        pause
        exit /b 1
    )
...
if %errorlevel% neq 0 (
    echo.
    echo WARNING: npm install had errors. You may need to run it again.
)
echo.
...
if %errorlevel% neq 0 (
    echo.
    echo WARNING: Playwright install had errors. The poster may not work.
)
echo.
